...
final class AllDayEventView: UIView {
    ...
    init(events: [Event], frame: CGRect, style: AllDayStyle, date: Date?) {
        ...
        let startEvents = events.map({ AllDayEvent(id: $0.ID, text: $0.text, date: $0.start, color: Event.Color($0.backgroundColor).value) })
        let endEvents = events.map({ AllDayEvent(id: $0.ID, text: $0.text, date: $0.end, color: Event.Color($0.backgroundColor).value) })
